...
export type cHouraiDataStructure = [
  { name: 'ID', value: number },
  { name: 'name', value: string },
  { name: 'npcID', value: number },
];
@Component({
  selector: 'app-c-hourai-data',
  templateUrl: './c-hourai-data.component.html',
  styleUrls: ['./c-hourai-data.component.scss']
})
export class CHouraiDataComponent {
  contentJson: string = "";
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'npcID', value: this.selectedItem[2].value },
      ];
    }
    else {
      this.editingItem = [
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: cHouraiDataStructure) => {
        // Write the opening tag for the item
        xml += '	<object name="MiCHouraiData">\n';
        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="name"><![CDATA[${item[1].value || ''}]]></member>\n`;
        xml += `    <member name="npcID">${item[2].value}</member>\n`;
