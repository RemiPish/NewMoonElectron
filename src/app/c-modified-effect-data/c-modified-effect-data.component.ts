...
export type cModifiedEffectDataStructure = [
...
@Component({
  selector: 'app-c-modified-effect-data',
  templateUrl: './c-modified-effect-data.component.html',
  styleUrls: ['./c-modified-effect-data.component.scss']
})
export class CModifiedEffectDataComponent {
  contentJson: string = "";
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'sequenceID', value: this.selectedItem[1].value },
        { name: 'name', value: this.selectedItem[2].value },
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: cModifiedEffectDataStructure) => {
        // Write the opening tag for the item

        xml += '	<object name="MiCModifiedEffectData">\n';
        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="sequenceID">${item[1].value}</member>\n`;
        xml += `    <member name="name"><![CDATA[${item[2].value || ''}]]></member>\n`;
