...
export type sItemDataStructure = [
  { name: 'ID', value: number },
  { name: 'tokusei', value: number[] }
];

@Component({
  selector: 'app-s-item-data',
  templateUrl: './s-item-data.component.html',
  styleUrls: ['./s-item-data.component.scss']
})
export class SItemDataComponent {
  contentJson: string = "";
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      for (let i = 0; i < this.selectedItem[1].value.length; i++) {
        tokusei.push(this.selectedItem[1].value[i]);
      }
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: sItemDataStructure) => {
        // Write the opening tag for the item
        xml += '	<object name="MiSItemData">\n';
