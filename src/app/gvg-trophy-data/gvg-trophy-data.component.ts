...
export type gvgTrophyDataStructure = [
  { name: 'ID', value: number },
  { name: 'mode', value: number },
...
@Component({
  selector: 'app-gvg-trophy-data',
  templateUrl: './gvg-trophy-data.component.html',
  styleUrls: ['./gvg-trophy-data.component.scss']
})
export class GvgTrophyDataComponent {
  contentJson: string = "";
  ...
  onSearch() {
    ...
    this.filteredContent = this.content.filter((item: { value: string; }[]) => {
      ...
      const nameStr = String(item[3].value).toLowerCase();
      const desStr = String(item[4].value).toLowerCase();
  ...
  async parseData(json: string) {
    try {
      ...
      if (parsed.length) {
        ...
        items = await Promise.all(parsed.map(async (item: any) => {
          return [
            ...
            { name: "mode", value: item.member.find((m: any) => m["@name"] === "mode")["#text"] },
            { name: "bonus", value: item.member.find((m: any) => m["@name"] === "bonus")["#text"] },
      ...
      { name: "mode", value: parsed.member.find((m: any) => m["@name"] === "mode")["#text"] },
      { name: "bonus", value: parsed.member.find((m: any) => m["@name"] === "bonus")["#text"] },
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'mode', value: this.selectedItem[1].value },
        { name: 'bonus', value: this.selectedItem[2].value },
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: gvgTrophyDataStructure) => {

        xml += '  <object name="MiGvGTrophyData">\n';

        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="mode">${item[1].value}</member>\n`;
        xml += `    <member name="bonus">${item[2].value}</member>\n`;
