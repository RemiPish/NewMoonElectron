...
export type questBonusCodeDataStructure = [
...
@Component({
  selector: 'app-quest-bonus-code-data',
  templateUrl: './quest-bonus-code-data.component.html',
  styleUrls: ['./quest-bonus-code-data.component.scss']
})
export class QuestBonusCodeDataComponent {
  contentJson: string = "";
      ...
      if (parsed.length) {
        ...
        items = await Promise.all(parsed.map(async (item: any) => {
          return [
            ...
            { name: "titleID", value: item.member.find((m: any) => m["@name"] === "titleID")["#text"] }
          ];
        }));
      }
      else items = [[{ name: "ID", value: parsed.member.find((m: any) => m["@name"] === "ID")["#text"] },
      { name: 'count', value: parsed.member.find((m: any) => m["@name"] === "unknown1")["#text"] },
      { name: 'titleID', value: parsed.member.find((m: any) => m["@name"] === "titleID")["#text"] }
      ]];
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'count', value: this.selectedItem[1].value },
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: questBonusCodeDataStructure) => {
        // Write the opening tag for the item

        xml += '	<object name="MiQuestBonusCodeData">\n';
        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="count">${item[1].value}</member>\n`;
