...
export type cIconDataStructure = [
...
@Component({
  selector: 'app-c-icon-data',
  templateUrl: './c-icon-data.component.html',
  styleUrls: ['./c-icon-data.component.scss']
})
export class CIconDataComponent implements OnChanges {
  contentJson: string = "";
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'value', value: this.selectedItem[1].value },
      ];
    }
    else {
      this.editingItem = [
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: cIconDataStructure) => {
        // Write the opening tag for the item
        xml += '	<object name="MiCIconData">\n';
        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="value"><![CDATA[${item[1].value || ''}]]></member>\n`;
