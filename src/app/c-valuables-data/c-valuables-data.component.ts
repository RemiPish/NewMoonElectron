...
export type cValuablesDataStructure = [
...
@Component({
  selector: 'app-c-valuables-data',
  templateUrl: './c-valuables-data.component.html',
  styleUrls: ['./c-valuables-data.component.scss']
})
export class CValuablesDataComponent {
  contentJson: string = "";
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'sortOrder', value: this.selectedItem[1].value },
        { name: 'name', value: this.selectedItem[2].value },
        { name: 'desc', value: this.selectedItem[3].value },
        { name: 'hidden', value: this.selectedItem[4].value },
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: cValuablesDataStructure) => {
        // Write the opening tag for the item
        xml += '  <object name="MiCValuablesData">\n';
        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="sortOrder">${item[1].value}</member>\n`;
        xml += `    <member name="name"><![CDATA[${item[2].value || ''}]]></member>\n`;
        xml += `    <member name="desc"><![CDATA[${item[3].value || ''}]]></member>\n`;
        xml += `    <member name="hidden">${item[4].value}</member>\n`;
