...
export type cHouraiMessageDataStructure = [
...
@Component({
  selector: 'app-c-hourai-message-data',
  templateUrl: './c-hourai-message-data.component.html',
  styleUrls: ['./c-hourai-message-data.component.scss']
})
export class CHouraiMessageDataComponent {
  contentJson: string = "";
  ...
  async parseData(json: string) {
    try {
      ...
      if (parsed.length) {
        items = await Promise.all(parsed.map(async (item: any) => {
          return [
            ...
            { name: "message", value: item.member.find((m: any) => m["@name"] === "message")["#text"] },
      ...
      { name: "message", value: parsed.member.find((m: any) => m["@name"] === "message")["#text"] },
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'message', value: this.selectedItem[1].value },
        { name: 'soundID', value: this.selectedItem[2].value },
      ];
    }
    else {
      this.editingItem = [
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: cHouraiMessageDataStructure) => {
        // Write the opening tag for the item
        xml += '	<object name="MiCHouraiMessageData">\n';
        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="message"><![CDATA[${item[1].value || ''}]]></member>\n`;
        xml += `    <member name="soundID">${item[2].value}</member>\n`;
