...
export class TankDataComponent {
  contentJson: string = "";
  ...
  onSearch() {
    ...
    this.filteredContent = this.content.filter((item: { value: string; }[]) => {
      ...
      const itemidStr = String(item[1].value).toLowerCase();
      return idStr.includes(searchStr) || itemidStr.includes(searchStr);
    });
  ...
  async parseData(json: string) {
    try {
      ...
      if (parsed.length) {
        ...
        items = await Promise.all(parsed.map(async (item: any) => {
          return [
            ...
            { name: "maxStack", value: item.member.find((m: any) => m["@name"] === "maxStack")["#text"] }
          ];
        }));
      }
      ...
      { name: 'maxStack', value: parsed.member.find((m: any) => m["@name"] === "maxStack")["#text"] }
      ]];
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'maxStack', value: this.selectedItem[2].value }
      ];
    }
    else {
      this.editingItem = [
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: tankDataStructure) => {
        // Write the opening tag for the item

        xml += '	<object name="MiTankData">\n';

        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="itemID  ">${item[1].value}</member>\n`;
        xml += `    <member name="maxStack">${item[2].value}</member>\n`;
