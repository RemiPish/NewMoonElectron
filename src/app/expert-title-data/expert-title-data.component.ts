...
export type expertTitleDataStructure = [
  { name: 'ID', value: number },
  { name: 'unknown', value: number[] }
];

@Component({
  selector: 'app-expert-title-data',
  templateUrl: './expert-title-data.component.html',
  styleUrls: ['./expert-title-data.component.scss']
})
export class ExpertTitleDataComponent {
  contentJson: string = "";
    ...
    try {
      ...
      if (parsed.length) {
        const items = await Promise.all(parsed.map(async (item: any) => {
          let messageIDs: any[] = [];
          for (let i = 0; i < item.member.find((m: any) => m["@name"] === "unknown").element.length; i++) {
            messageIDs.push(item.member.find((m: any) => m["@name"] === "unknown").element[i]);
          }
          return [
            { name: "ID", value: item.member.find((m: any) => m["@name"] === "ID")["#text"] },
            { name: 'unknown', value: messageIDs },
          ];
        }));
        console.log(items);
        return items;
        ...
        let messageIDs: any[] = [];
        for (let i = 0; i < parsed.member.find((m: any) => m["@name"] === "unknown").element.length; i++) {
          messageIDs.push(parsed.member.find((m: any) => m["@name"] === "unknown").element[i]);
        }
        return [[
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: expertTitleDataStructure) => {
        // Write the opening tag for the item
        xml += '	<object name="MiExpertTitleData">\n';
