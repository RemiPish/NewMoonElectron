...
export type mitamaReunionBonusDataStructure = [
...
@Component({
  selector: 'app-mitama-reunion-bonus-data',
  templateUrl: './mitama-reunion-bonus-data.component.html',
  styleUrls: ['./mitama-reunion-bonus-data.component.scss']
})
export class MitamaReunionBonusDataComponent {
  contentJson: string = "";
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'value', value: this.selectedItem[2].value }
      ];
    }
    else {
      this.editingItem = [
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: mitamaReunionBonusDataStructure) => {
        // Write the opening tag for the item
        xml += '	<object name="MiMitamaReunionBonusData">\n';
        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="type">${item[1].value}</member>\n`;
        xml += `    <member name="value">${item[2].value}</member>\n`;
