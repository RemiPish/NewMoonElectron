...
export type cValuableStructure = [
  ...
  { name: 'nif', value: string },
];

@Component({
  selector: 'app-c-appearance-equip-data',
  templateUrl: './c-appearance-equip-data.component.html',
  styleUrls: ['./c-appearance-equip-data.component.scss']
})
export class CAppearanceEquipDataComponent implements OnChanges {
  contentJson: string = "";
  ...
  onSearch() {
    ...
    this.filteredContent = this.content.filter((item: { value: string; }[]) => {
      ...
      const nifStr = String(item[1].value).toLowerCase();
      return idStr.includes(searchStr) || nifStr.includes(searchStr);
    });
      ...
      if (parsed.length) {
        items = await Promise.all(parsed.map(async (item: any) => {
          return [
            ...
            { name: "nif", value: item.member.find((m: any) => m["@name"] === "nif")["#text"] },

          ];
        }));
      }
      else items = [[{ name: "ID", value: parsed.member.find((m: any) => m["@name"] === "ID")["#text"] },
      { name: "nif", value: parsed.member.find((m: any) => m["@name"] === "nif")["#text"] },
      ]];
  ...
  openEdition(id?: any) {
    ...
    if (id) {
      ...
      this.editingItem = [
        ...
        { name: 'nif', value: this.selectedItem[1].value },
      ];
    }
    else {
      this.editingItem = [
  ...
  writeXmlFile(saveMode: string) {
    ...
    if (confirmation) {
      ...
      this.content.forEach((item: cValuableStructure) => {
        ...
        xml += '	<object name="MiCAppearanceEquipData">\n';
        xml += `    <member name="ID">${item[0].value}</member>\n`;
        xml += `    <member name="nif"><![CDATA[${item[1].value || ''}]]></member>\n`;
